...
class AdHocCommands:
    ...
    def __init__(self, pytrans):
        ...
        self.pytrans.disco.addFeature(
            globals.COMMANDS, self.incomingIq, config.jid)
        self.pytrans.disco.addNode(
            globals.COMMANDS, self.sendCommandList, "command_CommandList", config.jid, True)
    ...
    def addCommand(self, command, handler, name, rights):
        ...
        self.pytrans.disco.addNode(
            command, self.incomingIq, name, config.jid, False)
        ...
        for child in el.elements():
            ...
            if child.name == "query" and xmlns == globals.DISCO_INFO:
                if node and self.commands.has_key(node) and itype == "get":
                    self.sendCommandInfoResponse(
                        to=fro, ID=ID, node=node, ulang=ulang)
                    handled = True
            elif child.name == "query" and xmlns == globals.DISCO_ITEMS:
                if node and self.commands.has_key(node) and itype == "get":
                    self.sendCommandItemsResponse(
                        to=fro, ID=ID, node=node, ulang=ulang)
                    handled = True
            ...
            if not handled:
                ...
                self.pytrans.iq.sendIqError(to=fro, fro=config.jid, ID=ID,
                                            xmlns=xmlns, etype="cancel",
    ...
    def sendCommandList(self, el):
        ...
        for command in self.commands:
            # user can view this item (enough rights)
            if self.commandRights[command] <= rights:
                item = query.addElement("item")
                item.attributes["jid"] = config.jid
                item.attributes["node"] = command
                item.attributes["name"] = lang.get(
                    self.commandNames[command], ulang)
