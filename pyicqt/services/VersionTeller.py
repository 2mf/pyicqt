...
class VersionTeller:
    ...
    def __init__(self, pytrans):
        ...
        self.pytrans.disco.addFeature(
            globals.IQVERSION, self.incomingIq, config.jid)
        self.pytrans.disco.addFeature(
            globals.IQVERSION, self.incomingIq, "USER")
        self.version = self.pytrans.version
        self.os = "Python " + \
            sys.version.split(
                ' ')[0] + "/" + sys.platform + ", Twisted " + twisted.copyright.version

    def incomingIq(self, el):
        eltype = el.getAttribute("type")
        if eltype != "get":
            return  # Only answer "get" stanzas
