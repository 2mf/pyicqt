...
class VCardFactory:
    ...
    def __init__(self, pytrans):
        ...
        self.pytrans.disco.addFeature(
            globals.VCARD, self.incomingIq, config.jid)
        self.pytrans.adhoc.addCommand(
            "updatemyvcard", self.getMyVCard, "command_UpdateMyVCard", rights_user)
    ...
    def incomingIq(self, el):
        ...
        if itype != "get" and itype != "error":
            self.pytrans.iq.sendIqError(
                to=fro, fro=config.jid, ID=ID, xmlns="vcard-temp", etype="cancel", condition="feature-not-implemented")
            return
        ...
        if not toGateway:
            if not self.pytrans.sessions.has_key(froj.userhost()):
                self.pytrans.iq.sendIqError(
                    to=fro, fro=config.jid, ID=ID, xmlns="vcard-temp", etype="auth", condition="not-authorized")
                return
            ...
            if not c:
                ...
                self.pytrans.legacycon.getvCardNotInList(
                    vCard, to).addCallback(self.gotvCardResponse, iq, filter)
                return
        ...
        if toGateway:
            ...
            c.fillvCard(vCard, to).addCallback(
                self.gotvCardResponse, iq, filter)
