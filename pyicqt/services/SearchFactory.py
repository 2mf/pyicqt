...
class SearchFactory:
    ...
    def __init__(self, pytrans):
        ...
        if 0:  # Disable for now
            self.pytrans.disco.addFeature(
                globals.IQSEARCH, self.incomingIq, config.jid)
    ...
    def incomingIq(self, el):
        ...
        if not hasattr(self.pytrans, "legacycon"):
            self.pytrans.iq.sendIqError(
                to=to, fro=config.jid, ID=ID, xmlns=globals.COMMANDS, etype="cancel", condition="service-unavailable")
        elif eltype == "get":
    ...
    def sendSearchForm(self, el):
        ...
        x.addChild(utils.makeDataFormElement(
            "text-single", "email", "E-Mail Address"))
        x.addChild(
            utils.makeDataFormElement("text-single", "first", "First Name"))
        x.addChild(utils.makeDataFormElement(
            "text-single", "middle", "Middle Name"))
        x.addChild(
            utils.makeDataFormElement("text-single", "last", "Last Name"))
        x.addChild(utils.makeDataFormElement(
            "text-single", "maiden", "Maiden Name"))
        x.addChild(
            utils.makeDataFormElement("text-single", "nick", "Nickname"))
        x.addChild(utils.makeDataFormElement(
            "text-single", "address", "Street Address"))
        ...
        x.addChild(
            utils.makeDataFormElement("text-single", "country", "Country"))
        x.addChild(
            utils.makeDataFormElement("text-single", "interest", "Interest"))
