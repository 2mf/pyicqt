    ...
    def __init__(self, pytrans):
        ...
        if not config.disableAvatars and not config.disableIQAvatars:
            self.pytrans.disco.addFeature(
                globals.IQAVATAR, self.incomingIq, "USER")
            self.pytrans.disco.addFeature(
                globals.STORAGEAVATAR, self.incomingIq, "USER")
    ...
    def incomingIq(self, el):
        ...
        if not xmlns:
            self.pytrans.iq.sendIqError(
                to=fro, fro=config.jid, ID=ID, xmlns=xmlns, etype="cancel", condition="bad-request")
            return
        if itype != "get" and itype != "error":
            self.pytrans.iq.sendIqError(
                to=fro, fro=config.jid, ID=ID, xmlns=xmlns, etype="cancel", condition="feature-not-implemented")
            return
        ...
        if not self.pytrans.sessions.has_key(froj.userhost()):
            self.pytrans.iq.sendIqError(
                to=fro, fro=config.jid, ID=ID, xmlns=xmlns, etype="auth", condition="not-authorized")
            return
        ...
        if not c:
            self.pytrans.iq.sendIqError(
                to=fro, fro=config.jid, ID=ID, xmlns=xmlns, etype="cancel", condition="recipient-unavailable")
            return
