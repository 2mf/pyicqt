...
class RosterRetrieval:
    ...
    def __init__(self, pytrans):
        ...
        self.pytrans.adhoc.addCommand(
            "retrieveroster", self.incomingIq, "command_RosterRetrieval", rights_user)
    ...
    def incomingIq(self, el):
        ...
        if not self.pytrans.sessions.has_key(froj.userhost()):
            self.pytrans.iq.sendIqError(
                to=fro, fro=config.jid, ID=ID, xmlns=globals.COMMANDS, etype="cancel", condition="service-unavailable")
            return
