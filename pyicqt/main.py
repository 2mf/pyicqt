...
from pyicqt import debug

from pyicqt import config
from pyicqt import xmlconfig
conffile = "config.xml"
profilelog = None
options = {}
daemonizeme = False
debugLevel = 0
debugFile = ""
opts, args = getopt.getopt(sys.argv[1:], "bc:o:dDgtl:p:h", [
                           "background", "config=", "option=", "debug", "Debug", "garbage", "traceback", "log=", "profile=", "help"])
for o, v in opts:
    if o in ("-c", "--config"):
        conffile = v
    elif o in ("-p", "--profile"):
...
from pyicqt import xdb
from pyicqt import avatar
from pyicqt import session
from pyicqt import git_ver
from pyicqt import jabw
from pyicqt import iq
from pyicqt import disco
from pyicqt import adhoc
from pyicqt import pubsub
from pyicqt import register
...
class PyTransport(component.Service):
    ...
    def loopFunc(self):
        ...
        self.serviceplugins['Statistics'].stats[
        ...
        if numsessions > 0:
            ...
            for key in oldDict:
                ...
                if not session.alive:
                    ...
                    # Don't add it to the new dictionary. Effectively removing
                    # it
                else:
    ...
    def componentConnected(self, xmlstream):
        ...
        self.xmlstream.addObserver(
            "/error[@xmlns='http://etherx.jabber.org/streams']", self.streamError)
    ...
    def onRouteMessage(self, el):
        ...
        for child in el.elements():
            if child.name == "message":
                ...
                if child.getAttribute("to") and child.getAttribute("to").find("@-internal") > 0:
                    return
        ...
        if self.sessions.has_key(froj.userhost()):
            ...
            for child in el.elements():
                if child.name == "body":
                    body = child.__str__()
                    LogEvent(
                        INFO, msg="Sending error response to a message outside of seession")
                    jabw.sendErrorMessage(
                        self, fro, to, "auth", "not-authorized", lang.get("notloggedin", ulang), body)
    ...
    def onPresence(self, el):
        ...
        if self.sessions.has_key(froj.userhost()):
            ...
            if to.find('@') < 0:
                # If the presence packet is to the transport (not a user) and
                # there isn't already a session
                # Don't create a session unless they're sending available
                # presence
                if not ptype:
                    if self.serviceplugins['Statistics'].stats["OnlineSessions"] < 1000:
                        LogEvent(
                            INFO, msg="Attempting to create a new session")
                        s = session.makeSession(
                            self, froj.userhost(), ulang, toj)
                        if s:
                            ...
                            jabw.sendMessage(
                                self, to=froj.userhost(), fro=config.jid, body=lang.get("notregistered", ulang))
                    else:
                        LogEvent(INFO, msg="Max sessions limit reached")
                        jabw.sendMessage(
                            self, to=froj.userhost(), fro=config.jid, body=lang.get("sessionslimit", ulang))

                elif ptype != "error":
                    LogEvent(
                        INFO, msg="Sending unavailable presence to non-logged in user")
                    pres = Element((None, "presence"))
                ...
                if s:
                    ...
                    s.handleResourcePresence(froj.userhost(), froj.resource, toj.userhost(
                    ), toj.resource, 0, None, None, None, None)
    ...
    def sendMessagesOnShutdown(self):
        if config.enableShutdownMessage:
            ...
            for jid in self.sessions:
                ...
                jabw.sendErrorMessage(
                    self, jid, tmpjid, 'cancel', 'undefined-condition', '', msg)
    ...
    def __init__(self):
        ...
        if config.saslUsername:
            ...
            self.c = sasl.buildServiceManager(
                jid, config.saslUsername, config.secret, "tcp:%s:%s" % (config.mainServer, config.port))
        else:
    ...
    if config.webport:
        try:
            ...
            site = appserver.NevowSite(
                web.WebInterface(pytrans=app.transportSvc))
            reactor.listenTCP(config.webport, site)
            LogEvent(INFO, msg="Web interface activated")
        except:
            LogEvent(
                WARN, msg="Unable to start web interface.  Either Nevow is not installed or you need a more recent version of Twisted.  (>= 2.0.0.0)")
    reactor.run()
