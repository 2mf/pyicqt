...
from pyicqt.legacy import glue
...
class BuddyList:
    ...
    def __init__(self, session):
        ...
        for c in self.xdbcontacts:
            ...
            if self.xdbcontacts[c].has_key("nickname"):
                jabContact.updateNickname(
                    self.xdbcontacts[c]["nickname"], push=False)
            if not config.disableAvatars:
                if self.xdbcontacts[c].has_key("shahash"):
                    LogEvent(
                        INFO, self.session.jabberID, "Setting custom avatar for %s" % (c))
                    ...
                    if not config.disableDefaultAvatar:
                        LogEvent(
                            INFO, self.session.jabberID, "Setting default avatar for %s" % (c))
                        if c[0].isdigit():
                            jabContact.updateAvatar(
                                glue.defaultICQAvatar, push=False)
                        else:
                            jabContact.updateAvatar(
                                glue.defaultAIMAvatar, push=False)
    ...
    def setCustomStatus(self, contact, customStatus):
        ...
        # if custom status for user exists
        if lo_contact in self.usercustomstatuses and len(self.usercustomstatuses[lo_contact]) > 0:
            for key in customStatus:  # for all keys
                self.usercustomstatuses[lo_contact][
                    key] = customStatus[key]  # update it!
        else:
            # copy full customStatus
            self.usercustomstatuses[lo_contact] = customStatus

    def delCustomStatus(self, contact, delmask=None, savemask=None):
        LogEvent(INFO, self.session.jabberID)
        lo_contact = contact.lower()
        # if custom status for user exists
        if lo_contact in self.usercustomstatuses:
    ...
    def setUserVars(self, contact, uvars):
        LogEvent(INFO, self.session.jabberID)
        lo_contact = contact.lower()
        # if vars for user exists
        if lo_contact in self.uservars and len(self.uservars[lo_contact]) > 0:
    ...
    def diffAvatar(self, contact, md5Hash=None, numHash=None):
        if self.xdbcontacts.has_key(contact.lower()):
            LogEvent(INFO, self.session.jabberID, "Comparing [M] %r and %r, [N] %r and %r" % (md5Hash, self.xdbcontacts[
                     contact.lower()].get("md5hash", None), str(numHash), self.xdbcontacts[contact.lower()].get("numhash", None)))
    ...
    def updateSSIContact(self, contact, presence="unavailable", show=None, status=None, nick=None, ipaddr=None, lanipaddr=None, lanipport=None, icqprotocol=None, url=None):
        ...
        if not c:
            LogEvent(INFO, self.session.jabberID,
                     "Update setting default avatar for %s" % (contact))
            c = self.session.contactList.createContact(
                icq2jid(contact), "both")
            if not config.disableAvatars:
                if contact[0].isdigit():
        ...
        if not self.xdbcontacts.has_key(contact.lower()):
            ...
            if nick:
                ...
                self.session.sendRosterImport(
                    icq2jid(contact), "subscribe", "both", contact)
            self.session.pytrans.xdb.setListEntry(
                "roster", self.session.jabberID, contact.lower(), payload=self.xdbcontacts[contact.lower()])
        else:
            decodednickname = self.xdbcontacts[
                contact.lower()].get('nickname', '')
            try:
    ...
    def getBuddyList(self):
        LogEvent(INFO, self.session.jabberID)
        bl = dict()
        entities = self.session.pytrans.xdb.getList(
            "roster", self.session.jabberID)
        if entities == None:
            LogEvent(
                INFO, self.session.jabberID, "Unable to get list, or empty")
            return bl
