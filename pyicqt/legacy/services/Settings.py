...
class Settings:
    ...
    def __init__(self, pytrans):
        ...
        self.pytrans.adhoc.addCommand(
            'settings', self.incomingIq, 'command_Settings', rights_user)
    ...
    def incomingIq(self, el):
                                ...
                                for value in field.elements():
                                    if value.name == 'value':
                                        if field.getAttribute('type') == 'text-multi' and field.getAttribute('var') in settings_dict:
                                            settings_dict[
                                                field.getAttribute('var')] += '\n%s' % value.__str__()
                                        else:
                                            settings_dict[
                                                field.getAttribute('var')] = value.__str__()

        if jid not in self.pytrans.sessions:  # if user not logined
            self.pytrans.adhoc.sendError('settings', el, errormsg=lang.get(
        ...
        # if user not connected to ICQ network
            ...
            self.pytrans.adhoc.sendCancellation(
                "settings", el, sessionid)  # correct cancel handling
        elif stage == '2' or do_action == 'done':
            if settings_page == 'xstatus_settings':
                # apply x-status settings
                self.ApplyXstatusSettings(toj, settings_dict)
            elif settings_page == 'clist_settings':
                # apply contact list settings
                self.ApplyContactListSettings(toj, settings_dict)
            elif settings_page == 'message_settings':
                # apply message settings
                self.ApplyMessageSettings(toj, settings_dict)
            elif settings_page == 'personal_events_settings':
                # apply personal events settings
                self.ApplyPersonalEventsSettings(toj, settings_dict)
            elif settings_page == 'autoanswer_settings':
                # apply auto-answer settings
                self.ApplyAutoanswerSettings(toj, settings_dict)
            self.sendCompletedForm(el, sessionid)  # send answer
        elif stage == '1':
            if settings_page == 'xstatus_settings':
                # send form with x-status settings
                self.sendXstatusSettingsForm(el, sessionid)
            elif settings_page == 'clist_settings':
                # send form with contact list settings
                self.sendContactListSettingsForm(el, sessionid)
            elif settings_page == 'message_settings':
                # send form with message settings
                self.sendMessageSettingsForm(el, sessionid)
            elif settings_page == 'personal_events_settings':
                # send form with personal events settings
                self.sendPersonalEventsSettingsForm(el, sessionid)
            elif settings_page == 'autoanswer_settings':
                # send form with auto-answer settings
                self.sendAutoanswerSettingsForm(el, sessionid)
        else:
    ...
    def sendSettingsClassForm(self, el, sessionid=None):
        ...
        option = field.addElement('option')
        option.attributes['label'] = lang.get(
            'settings_category_personal_events')
    ...
    def sendXstatusSettingsForm(self, el, sessionid=None):
        ...
        if config.xstatusessupport:
            ...
            field.addElement(
                'value', None, str(settings['away_messages_sending']))
            field.addElement(
                'desc', None, lang.get('away_messages_sending_Desc'))
            ...
            field.addElement(
                'value', None, str(settings['away_messages_receiving']))
            field.addElement(
                'desc', None, lang.get('away_messages_receiving_Desc'))
            ...
            field.addElement(
                'value', None, str(settings['xstatus_sending_mode']))
            ...
            field.addElement(
                'value', None, str(settings['xstatus_saving_enabled']))
            field.addElement(
                'desc', None, lang.get('xstatus_restore_after_disconnect_Desc'))
            ...
            field.addElement(
                'value', None, str(settings['xstatus_receiving_mode']))
            ...
            field.addElement(
                'value', None, str(settings['xstatus_option_smooth']))
            field.addElement(
                'desc', None, lang.get('xstatus_option_smooth_Desc'))
            ...
            field.addElement(
                'value', None, str(settings['xstatus_display_icon_as_PEP']))
            field.addElement(
                'desc', None, lang.get('xstatus_display_icon_as_PEP_Desc'))
            ...
            field.addElement(
                'value', None, str(settings['xstatus_display_text_as_PEP']))
            field.addElement(
                'desc', None, lang.get('xstatus_display_text_as_PEP_Desc'))
            ...
            field.addElement(
                'value', None, str(settings['xstatus_icon_for_transport']))
            field.addElement(
                'desc', None, lang.get('xstatus_icon_for_transport_Desc'))
    ...
    def sendContactListSettingsForm(self, el, sessionid=None):
        ...
        field.addElement(
            'value', None, str(settings['clist_show_phantombuddies']))
        field.addElement(
            'desc', None, lang.get('settings_clist_show_phantombuddies_Desc'))
        ...
        field.addElement(
            'value', None, str(settings['clist_deny_all_auth_requests']))
        field.addElement(
            'desc', None, lang.get('settings_clist_deny_all_auth_requests_Desc'))
    ...
    def sendMessageSettingsForm(self, el, sessionid=None):
        ...
        # encoding not from list
        if str(settings['userencoding_list']) not in userencoding_list.values() and str(settings['userencoding_list']) != config.encoding:
            option = field.addElement('option')
            option.attributes['label'] = '%s (%s)' % (
                lang.get('userencoding_list_other'), str(settings['userencoding_list']))
            option.addElement(
                'value', None, str(settings['userencoding_list']))
        # encoding from config
        if config.encoding not in userencoding_list.values():
            option = field.addElement('option')
            option.attributes['label'] = '%s (%s)' % (
                lang.get('userencoding_list_default'), config.encoding)
        ...
        for title in utf8_messages_sendmode:
            ...
            option.attributes['label'] = lang.get(title)
            option.addElement(
                'value', None, str(utf8_messages_sendmode[title]))
        field.addElement(
            'value', None, str(settings['utf8_messages_sendmode']))
        ...
        for title in offline_messages_sendenc:
            ...
            option.attributes['label'] = lang.get(title)
            option.addElement(
                'value', None, str(offline_messages_sendenc[title]))
        field.addElement(
            'value', None, str(settings['offline_messages_sendenc']))
        field.addElement(
            'desc', None, lang.get('offline_messages_sendenc_Desc'))
    ...
    def sendPersonalEventsSettingsForm(self, el, sessionid=None):
        ...
        x.attributes = {'xmlns': 'jabber:x:data', 'type': 'form'}
        x.addElement(
            'title', None, lang.get('settings_category_personal_events'))
        ...
        field.addElement(
            'value', None, str(settings['user_activity_receiving']))
        field.addElement(
            'desc', None, lang.get('user_activity_receiving_Desc'))
    ...
    def sendAutoanswerSettingsForm(self, el, sessionid=None):
        ...
        field.addElement(
            'value', None, str(settings['autoanswer_hide_dialog']))
    ...
    def ApplyXstatusSettings(self, to_jid, settings):
        ...
        if jid in self.pytrans.sessions:
            for key in settings:
                ...
                if config.xstatusessupport:
                    # disable sending of x-statuses
                    if key == 'xstatus_sending_mode' and str(settings[key]) == '0':
                        # keep values for mood/activity
                        mask = (
                            'mood', 'activity', 'subactivity', 'text', 'usetune')
                        bos.oscarcon.delSelfCustomStatus(savemask=mask)
                        bos.updateSelfXstatus()
                    if key == 'xstatus_receiving_mode' and str(settings[key]) == '0':
                        # fast redrawing for all status messages (need exclude
                        ...
                        for contact in contacts:
                            if contacts[contact]['show']:
                                saved_snac = legacycon.getSavedSnac(
                                    str(contact))
                                if saved_snac != '':
                                    legacycon.bos.updateBuddy(
                                        legacycon.bos.parseUser(saved_snac), True)
                    if key == 'xstatus_icon_for_transport':
                        if str(settings[key]) == '1':
                            # show icon for transport
                            bos.setStatusIconForTransport()
                        else:
                            # hide icon for transport
                            bos.setStatusIconForTransport(reset=True)
