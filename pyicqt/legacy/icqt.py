...
from pyicqt.tlib import oscar, socks5
...
class B(oscar.BOSConnection):
    ...
    def __init__(self, username, cookie, oscarcon):
        ...
        self.capabilities = [oscar.CAP_ICON, oscar.CAP_UTF,
                             oscar.CAP_ICQXTRAZ, oscar.CAP_SERV_REL, oscar.CAP_TYPING]
    ...
    def gotUserInfo(self, id, type, userinfo):
        if userinfo:
            for i in xrange(len(userinfo)):
                try:
                    userinfo[i] = userinfo[i].decode(
                        self.userEncoding, 'strict')
                ...
                if u.name == uin:
                    if u.authorized:
                        self.session.sendPresence(
                            to=self.session.jabberID, fro=icq2jid(uin), show=None, ptype="subscribed")
                        return
        ...
        if success:
            ...
            for g in self.ssigroups:
                for u in g.users:
                    if u.name == uin:
                        u.authorizationRequestSent = False
            self.session.sendPresence(to=self.session.jabberID, fro=icq2jid(
                uin), show=None, status=None, ptype="unsubscribed")

    def gotAuthorizationRequest(self, uin):
        from glue import icq2jid
        LogEvent(INFO, self.session.jabberID)
        # deny all auth requests
        if self.settingsOptionEnabled('clist_deny_all_auth_requests'):
            ...
            if not uin in self.authorizationRequests:
                ...
                self.session.sendPresence(
                    to=self.session.jabberID, fro=icq2jid(uin), ptype="subscribe")
    ...
    def sendPersonalEvents(self, buddyjid, mood, s_mood, act, s_act, subact, s_subact, text, s_text, usetune, s_usetune):
        ...
        if int(self.settingsOptionEnabled('user_mood_receiving')):
            # if need set other mood or other text
            if mood and (mood != s_mood or (s_mood and text != s_text)):
                self.session.pytrans.pubsub.sendMood(
                    to=to, fro=fro, mood=mood, text=text)  # send mood
                a_mood = mood
            elif s_mood and not mood:  # mood was set and not set now
                self.session.pytrans.pubsub.sendMood(
                    to=to, fro=fro, action='retract')  # retract mood
        # sending activity
        if int(self.settingsOptionEnabled('user_activity_receiving')):
            # if need set other act/subact or other text
            if act and (not (act == s_act and subact == s_subact) or (s_act and text != s_text)):
                self.session.pytrans.pubsub.sendActivity(
                    to=to, fro=fro, act=act, subact=subact, text=text)  # send act
                a_act = act
                a_subact = subact
            # act or subact was set and not set now
            elif s_act and not act or s_subact and not subact:
                self.session.pytrans.pubsub.sendActivity(
                    to=to, fro=fro, action='retract')  # retract activity
        # sending tune
        if int(self.settingsOptionEnabled('user_tune_receiving')):
            ...
            if usetune and ((s_usetune and text != s_text) or not s_usetune):
                self.session.pytrans.pubsub.sendTune(
                    to=to, fro=fro, musicinfo=musicinfo)  # send tune
                a_usetune = usetune
            elif s_usetune and not usetune:  # tune was set and not set now
                self.session.pytrans.pubsub.sendTune(
                    to=to, fro=fro, stop=True)  # stop tune
    ...
    def updateBuddy(self, user, selfcall=False):
        ...
        if encoding and status:
            ...
            # this is a fallback solution in case that the client status
            # encoding has not been extracted, to avoid raising an exception
        ...
        if user.iconmd5sum != None and not config.disableAvatars and not config.avatarsOnlyOnChat:
            if self.oscarcon.legacyList.diffAvatar(user.name, md5Hash=binascii.hexlify(user.iconmd5sum)):
                LogEvent(
                    INFO, self.session.jabberID, "Retrieving buddy icon for %s" % user.name)
                self.retrieveBuddyIconFromServer(
                    user.name, user.iconmd5sum, user.icontype).addCallback(self.gotBuddyIconFromServer)
            else:
                LogEvent(INFO, self.session.jabberID,
                         "Buddy icon is the same, using what we have for %s" % user.name)

        if user.caps:
            self.oscarcon.legacyList.setCapabilities(user.name, user.caps)
        if (user.customStatus and len(user.customStatus) > 0) or anstatus:
            self.oscarcon.legacyList.setCustomStatus(
                user.name, user.customStatus)
        else:
        ...
        if config.xstatusessupport:
            if int(self.settingsOptionValue('xstatus_receiving_mode')) != 0:
                ...
                if int(self.settingsOptionValue('xstatus_receiving_mode')) == 3:
                    # already have icon and text of a status
                    if 'icqmood' in user.customStatus and status != '':
                        requestForXStatus = False
                        status6 = status
                    if selfcall == True:
                        status6 = ''
                if int(self.settingsOptionValue('xstatus_receiving_mode')) in (1, 3):
                    x_status_title, x_status_desc = self.oscarcon.getXStatusDetails(
                        user.name)
                ...
                s_mood, s_act, s_subact, s_text, s_usetune = self.oscarcon.getPersonalEvents(
                    user.name)  # get Personal Events
                ...
                if self.settingsOptionEnabled('xstatus_display_text_as_PEP') or self.settingsOptionEnabled('xstatus_display_icon_as_PEP'):
                    ...
                    if self.settingsOptionEnabled('xstatus_display_icon_as_PEP'):
                        # x-status or additional normal status presents
                        if x_status_name != '' or anstatus:
                            mood_a = None
                            act_a = None
                            subact_a = None
                            if anstatus in AN_STATUS_MAP:
                                mood_a, act_a, subact_a = AN_STATUS_MAP[
                                    anstatus]
                            ...
                            if not mood and mood_a:  # if no mood from x-status
                                # get mood from additional normal status
                                mood = mood_a
                            # if no activity from x-status
                            if not act and act_a:
                    ...
                    if self.settingsOptionEnabled('xstatus_display_text_as_PEP'):
                        if status != '' and int(self.settingsOptionValue('xstatus_receiving_mode')) in (2, 3):
                            ...
                            act_p, subact_p = self.parseAndSearchForActivity(
                                status)
                    ...
                    mood, act, subact, usetune = self.sendPersonalEvents(
                        buddyjid, mood, s_mood, act, s_act, subact, s_subact, text, s_text, usetune, s_usetune)
                else:  # displaying disabled
                    self.sendPersonalEventsStop(
                        buddyjid, s_mood, s_act, s_subact, s_usetune)

                # no icon in roster. Impossible recognize or displaying
                # disabled
                if not mood and not act and not subact and not usetune:
                    if int(self.settingsOptionValue('xstatus_receiving_mode')) in (1, 2, 3):
                        if status == '' and x_status_name != '':
                            # write standart name
                            status = lang.get(x_status_name)
                        elif status != '' and x_status_name != '':
                            if status != lang.get(x_status_name):
                                # or add it as prefix
                                status = lang.get(
                                    x_status_name) + ': ' + status
                        if anstatus and anstatus != '':
                            if status != '':
                                status = '\n' + status
                            status = lang.get(anstatus) + status

                self.oscarcon.setPersonalEvents(
                    user.name, mood, act, subact, text, usetune)  # set personal events
                # need send request for x-status details
                if selfcall == False and requestForXStatus == True:
                    if int(self.settingsOptionValue('xstatus_receiving_mode')) in (1, 3):
                        self.sendXstatusMessageRequestWithDelay(
                            user.name)  # request Xstatus message
            else:
        ...
        if requestForAwayMsg == True:
            self.getAway(user.name).addCallback(
                self.sendAwayPresence, user, show, status)
        else:
            if user.idleTime:
            ...
            self.oscarcon.legacyList.updateSSIContact(user.name, presence=ptype, show=show, status=status, ipaddr=user.icqIPaddy,
                                                      lanipaddr=user.icqLANIPaddy, lanipport=user.icqLANIPport, icqprotocol=user.icqProtocolVersion, url=url)

    def gotBuddyIconFromServer(self, (contact, icontype, iconhash, iconlen, icondata)):
        if config.disableAvatars:
            return
        LogEvent(INFO, self.session.jabberID, "%s: hash: %s, len: %d" %
                 (contact, binascii.hexlify(iconhash), iconlen))
        if iconlen > 0 and iconlen != 90:  # Some ICQ clients send crap
            self.oscarcon.legacyList.updateAvatar(
                contact, icondata, md5Hash=iconhash)
    ...
    def offlineBuddy(self, user):
        ...
        self.oscarcon.legacyList.updateSSIContact(
            user.name, presence=ptype, show=show, status=status)

        s_mood, s_act, s_subact, s_text, s_usetune = self.oscarcon.getPersonalEvents(
            user.name)  # get Personal Events
        self.sendPersonalEventsStop(
            buddyjid, s_mood, s_act, s_subact, s_usetune)  # stop them
        # del custom status struct for user
        self.oscarcon.legacyList.delCustomStatus(user.name)
        # reset personal events struct
        self.oscarcon.setPersonalEvents(user.name, None, None, None)
    ...
    def receiveMessage(self, user, multiparts, flags, delay=None):
        ...
        LogEvent(INFO, self.session.jabberID, "%s %s %s" %
                 (user.name, multiparts, flags))
        ...
        if len(multiparts[0]) > 1:
            if multiparts[0][1] == 'unicode':
                ...
                encoding = utils.guess_encoding_by_decode(
                    text, self.userEncoding, 'minimal')[1]  # try guess
            else:
        ...
        # send incoming message to jabber client
        if not (self.settingsOptionEnabled('autoanswer_enable') and self.settingsOptionEnabled('autoanswer_hide_dialog')):
        ...
        self.session.pytrans.serviceplugins[
            'Statistics'].stats['IncomingMessages'] += 1
        ...
        if self.settingsOptionEnabled('autoanswer_enable'):
            ...
            # send autoanswer reply to user
            if not self.settingsOptionEnabled('autoanswer_hide_dialog'):
                self.session.sendMessage(to=self.session.jabberID, fro=sourcejid, body='%s %s' % (
                    lang.get('autoanswer_prefix'), autoanswer_text), mtype=mtype, delay=delay, xhtml=xhtml)
            # send auto-answer text to contact
            self.sendMessage(user.name, autoanswer_text)
        if not config.disableAwayMessage and self.awayMessage and not "auto" in flags:
            if not self.awayResponses.has_key(user.name) or self.awayResponses[user.name] < (time.time() - 900):
                ...
                self.sendMessage(
                    user.name, "Away message: " + self.awayMessage, autoResponse=1)
    ...
    def receiveReceiptMsgReceived(self, user, cookie):
        # legacy client received own message
        LogEvent(INFO, self.session.jabberID)
        msg_query = self.oscarcon.getUserVarValue(
            user, 'wait_for_confirm_msg_query')
        if len(msg_query):
            if cookie in msg_query:
                # message id, resource and time of sending
                jabber_mid, resource, s_time = msg_query[cookie]
                from glue import icq2jid
                sourcejid = icq2jid(user)
                # send confirmation to jabber client
                self.session.sendMessage(
                    to=self.session.jabberID + '/' + resource, fro=sourcejid, receipt=True, mID=jabber_mid)
                uvars = {}
    ...
    def receiveTypingNotify(self, type, user):
        ...
        sourcejid = icq2jid(user.name)
        if type == "begin":
            self.session.sendTypingNotification(
                to=self.session.jabberID, fro=sourcejid, typing=True)
            self.session.sendChatStateNotification(
                to=self.session.jabberID, fro=sourcejid, state="composing")
        elif type == "idle":
            self.session.sendTypingNotification(
                to=self.session.jabberID, fro=sourcejid, typing=False)
            self.session.sendChatStateNotification(
                to=self.session.jabberID, fro=sourcejid, state="paused")
        elif type == "finish":
            self.session.sendTypingNotification(
                to=self.session.jabberID, fro=sourcejid, typing=False)
            self.session.sendChatStateNotification(
                to=self.session.jabberID, fro=sourcejid, state="active")
    ...
    def errorMessage(self, message):
        ...
        self.session.sendErrorMessage(
            to=self.session.jabberID, fro=tmpjid, etype="cancel", condition="recipient-unavailable", explanation=message)
    ...
    def emailNotificationReceived(self, addr, url, unreadnum, hasunread):
        ...
        if unreadnum > self.unreadmessages:
            ...
            self.session.sendMessage(to=self.session.jabberID, fro=config.jid, body=lang.get(
                "icqemailnotification", config.jid) % (diff, addr, url), mtype="headline")
    ...
    def sendAwayPresence(self, msg, user, show, pstatus):
        ...
        # online since time and idle time
        if msg[0] and msg[1] and len(msg[0]) == 4 and len(msg[1]) == 2:
            onlineSince = datetime.datetime.utcfromtimestamp(
                struct.unpack('!I', msg[0])[0])
            log.msg('User %s online since %sZ' %
                    (user.name, onlineSince.isoformat()))
            ...
            if status != None:
                ...
                if m != None:
                    ...
                    if charset == 'unicode-2-0':
                        ...
                        pass
                    elif charset == self.userEncoding:
                        pass
                    else:
                        LogEvent(
                            INFO, self.session.jabberID, "Unknown charset (%s) of buddy's away message" % msg[0])
    ...
    def receivedIconUploadRequest(self, iconhash):
        if config.disableAvatars:
            return
        LogEvent(INFO, self.session.jabberID, "%s" %
                 binascii.hexlify(iconhash))
        if hasattr(self.oscarcon, "myavatar"):
            LogEvent(INFO, self.session.jabberID, "I have an icon, sending it on, %d" % len(
                self.oscarcon.myavatar))
            self.uploadBuddyIconToServer(self.oscarcon.myavatar, len(
                self.oscarcon.myavatar)).addCallback(self.uploadedBuddyIconToServer)
    ...
    def receivedIconDirect(self, user, icondata):
        if config.disableAvatars:
            return
        LogEvent(INFO, self.session.jabberID,
                 "%s [%d]" % (user.name, user.iconlen))
        # Some ICQ clients send crap
        if user.iconlen > 0 and user.iconlen != 90:
            self.oscarcon.legacyList.updateAvatar(
                user.name, icondata, numHash=user.iconcksum)
    ...
    def readGroup(self, memberlist, parent=None):
        for member in memberlist:
            if isinstance(member, oscar.SSIGroup):
                LogEvent(INFO, self.session.jabberID, "Found group %s" %
                         (member.name))
                ...
                if member.nick:
                    try:
                        unick, uenc = utils.guess_encoding(
                            member.nick, self.userEncoding, 'minimal', mode=2)  # attempt to decode
                    except (UnicodeError, LookupError):  # no, error occured
                        unick = member.nick.encode(
                            'utf-8', 'replace')  # convert to utf-8
                else:
                    unick = None
                if parent:
                    LogEvent(INFO, self.session.jabberID, "Found user %r (%r) from group %r" % (
                        member.name, unick, parent.name))
                else:
                    LogEvent(INFO, self.session.jabberID, "Found user %r (%r) from master group" % (
                        member.name, unick))
                self.oscarcon.legacyList.updateSSIContact(
                    member.name, nick=unick)
                ...
                LogEvent(
                    INFO, self.session.jabberID, "Found unknown SSI entity: %r" % member)
    ...
    def gotBuddyList(self, l):
        ...
        if l is not None and l[5] is not None:
            for i in l[5]:
                LogEvent(INFO, self.session.jabberID, "Found icon %s" %
                         (str(i)))
                self.ssiiconsum.append(i)
        self.activateSSI()
        if l is not None and l[8] is not None and l[3] != "denysome":
            LogEvent(INFO, self.session.jabberID,
                     "Permissions not set in a compatible manner on SSI, switching to 'deny some'")
        ...
        if self.session.pytrans:
            self.session.sendPresence(to=self.session.jabberID, fro=tmpjid, show=self.oscarcon.savedShow,
                                      status=self.oscarcon.savedFriendly, url=self.oscarcon.savedURL)
        ...
        if int(self.settingsOptionValue('xstatus_sending_mode')) == 1:
            self.setExtendedStatusRequest(
                message='', setmsg=True)  # reset ICQ6 status
    ...
    def gotNickname(self, (nick, first, last, email), uin):
        ...
        if nick:
            try:  # may be
                unick, uenc = utils.guess_encoding(
                    nick, self.userEncoding, 'minimal', mode=2)  # attempt to decode
            except (UnicodeError, LookupError):  # no
                unick = nick.encode('utf-8', 'replace')  # convert to utf-8
            LogEvent(
                INFO, self.session.jabberID, "Found a nickname, lets update.")
    ...
    def setStatusIconForTransport(self, reset=False):
        ...
        # get Personal Events
        s_mood, s_act, s_subact, s_text, s_usetune = self.oscarcon.getSelfPersonalEvents()

        # possible show it
        if config.xstatusessupport and int(self.settingsOptionValue('xstatus_sending_mode')) != 0 and self.settingsOptionEnabled('xstatus_icon_for_transport'):
            x_status_name = ''
            ...
            mood, act, subact, usetune = self.sendPersonalEvents(
                None, mood, s_mood, act, s_act, subact, s_subact, text, s_text, usetune, s_usetune)
        else:
            self.sendPersonalEventsStop(
                None, s_mood, s_act, s_subact, s_usetune)

        self.oscarcon.setSelfPersonalEvents(
            mood, act, subact, text, usetune)  # set personal events
...
class OA(oscar.OscarAuthenticator):
    ...
    def __init__(self, username, password, oscarcon, deferred=None, icq=1):
        self.oscarcon = oscarcon
        self.BOSClass = B
        oscar.OscarAuthenticator.__init__(
            self, username, password, deferred, icq)

    def connectToBOS(self, server, port):
        if config.socksProxyServer:
            c = socks5.ProxyClientCreator(
                reactor, self.BOSClass, self.username, self.cookie, self.oscarcon)
