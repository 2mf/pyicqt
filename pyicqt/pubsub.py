...
class PublishSubscribe:
    ...
    def sendActivity(self, to=None, fro=None, act=None, subact=None, text=None, subact_xmlns=None, action=None,  extend_subact_xmlns=None):
        ...
        if extend_subact_xmlns and extend_subact_xmlns is True:
            # our xmlns for extended activity
            subact_xmlns = globals.EXT_SUBACTIVITY
        else:
            subact_xmlns = None
...
class PubSubStorage:
    ...
    def dir(self, jid, node):
        ...
        X = os.path.sep
        d = os.path.abspath(config.spooldir) + X + config.jid + X + \
            "pubsub" + X + utils.mangle(jid) + X + self.nodeToPath(node) + X
    ...
    def getNodeList(self, jid):
        ...
        pubsubbase = os.path.abspath(
            config.spooldir) + X + config.jid + X + "pubsub" + X + utils.mangle(jid)
