...
class ServiceDiscovery:
    ...
    def __init__(self, pytrans):
        ...
        self.addFeature(globals.RECEIPTS, None, config.jid)
        self.addFeature(globals.XCONFERENCE, None, config.jid)
        ...
        for query in el.elements():
            ...
            if xmlns.startswith(globals.DISCO) and node:
                if self.nodes.has_key(to) and self.nodes[to].has_key(node) and self.nodes[to][node][0] != None:
                    ...
                    # If the node we're browsing wasn't found, fall through and
                    # display the root disco
                    self.sendDiscoInfoResponse(
                        to=fro, ID=ID, ulang=ulang, jid=to)
                    return
            ...
            self.pytrans.iq.sendIqError(
                to=fro, fro=to, ID=ID, xmlns=xmlns, etype="cancel", condition="feature-not-implemented")
    ...
    def sendDiscoInfoResponse(self, to, ID, ulang, jid):
        ...
        for (category, ctype, name) in self.identities.get(searchjid, []):
            LogEvent(INFO, msg="Found identity %r %r %r" %
                     (category, ctype, name))
