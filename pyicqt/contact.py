...
class Contact:
    ...
    def updatePresence(self, show, status, ptype, force=False, tojid=None, url=None):
        updateFlag = (
            self.show != show or self.status != status or self.ptype != ptype or force)
        self.show = show
        self.status = status
        self.ptype = ptype
        self.url = url
    ...
    def sendSub(self, ptype):
        self.contactList.session.sendPresence(
            to=self.contactList.session.jabberID, fro=self.jid, ptype=ptype)
    ...
    def sendPresence(self, tojid=None):
        ...
        if not tojid:
            tojid = self.contactList.session.jabberID
        self.contactList.session.sendPresence(to=tojid, fro=self.jid, ptype=self.ptype, show=self.show,
                                              status=self.status, avatarHash=avatarHash, nickname=self.nickname, payload=[caps], url=self.url)

    def updateRoster(self, ptype):
        self.contactList.session.sendRosterImport(
            jid=self.jid, ptype=ptype, sub=self.sub, groups=self.groups)
