...
def authenticate_ldap(user, pwd):
    ...
    try:
        ...
        l.simple_bind_s(
            config.authRegister_LDAP["rootDN"], config.authRegister_LDAP["password"])
        LogEvent(INFO, msg="Bound to LDAP server")
        searchterm = config.authRegister_LDAP["uidAttr"] + "=" + user
        ldap_result_id = l.search(
            config.authRegister_LDAP["baseDN"], ldap.SCOPE_SUBTREE, searchterm, None)
    ...
    def __init__(self, pytrans):
        ...
        if not config.disableRegister:
            self.pytrans.disco.addFeature(
                globals.IQREGISTER, self.incomingRegisterIq, config.jid)
        LogEvent(INFO)
            ...
            if queryFind.name == "query":
                for child in queryFind.elements():
                    try:
                        ...
                        # Ignore any errors, we'll check everything below
                        continue
    ...
    def updateRegistration(self, incoming):
        ...
        if username and password and len(username) > 0 and len(password) > 0:
            ...
            try:
                ...
                jabw.sendPresence(
                    self.pytrans, to=to, fro=config.jid, ptype="subscribe")
                if config.registerMessage:
                    jabw.sendMessage(self.pytrans, to=incoming.getAttribute(
                        "from"), fro=config.jid, body=config.registerMessage)
            except:
                self.xdbErrorReply(incoming)
                raise
