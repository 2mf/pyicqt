...
class XDB:
    ...
    def getRegistration(self, jabberID):
        ...
        c = self.db.cursor()
        c.execute(
            "SELECT username,password,UNHEX(encryptedpassword) FROM register WHERE owner = '%s'" % jabberID)
        ret = c.fetchone()
        if ret:
    ...
    def setRegistration(self, jabberID, username, password):
        ...
        if config.xdbDriver_mysql.get("format", "") == "encrypted":
            c.execute("INSERT INTO register(owner,username,encryptedpassword) VALUES('%s','%s',HEX('%s'))" % (
                jabberID, username, password))
        else:
            c.execute("INSERT INTO register(owner,username,password) VALUES('%s','%s','%s')" % (
                jabberID, username, password))
    ...
    def getSettingList(self, jabberID):
        ...
        c = self.db.cursor()
        c.execute(
            "SELECT variable,value FROM settings WHERE owner = '%s'" % (jabberID))
        results = []
        ret = c.fetchone()
        while ret:
            (variable) = ret[0]
    ...
    def getSetting(self, jabberID, variable):
        ...
        self.db_ping()
        c = self.db.cursor()
        c.execute("SELECT value FROM settings WHERE owner = '%s' AND variable = '%s'" % (
            jabberID, variable))
        ret = c.fetchone()
        if ret:
            (value) = ret[0]
    ...
    def setSetting(self, jabberID, variable, value):
        ...
        self.db_ping()
        c = self.db.cursor()
        c.execute("DELETE FROM settings WHERE owner = '%s' AND variable = '%s'" % (
            jabberID, variable))
        c.execute("INSERT INTO settings(owner,variable,value) VALUES('%s','%s','%s')" % (
            jabberID, variable, value))

    def getCSetting(self, jabberID, variable):
        """ Gets a custom user setting from the XDB. """
        self.db_ping()
        c = self.db.cursor()
        c.execute("SELECT value FROM csettings WHERE owner = '%s' AND variable = '%s'" % (
            jabberID, variable))
        ret = c.fetchone()
        if ret:
            (value) = ret[0]
    ...
    def setCSetting(self, jabberID, variable, value):
        ...
        self.db_ping()
        c = self.db.cursor()
        c.execute("DELETE FROM csettings WHERE owner = '%s' AND variable = '%s'" % (
            jabberID, variable))
        c.execute("INSERT INTO csettings(owner,variable,value) VALUES('%s','%s','%s')" % (
            jabberID, variable, value))
    ...
    def getXstatusText(self, jabberID, number):
        ...
        self.db_ping()
        c = self.db.cursor()
        c.execute("SELECT title, value FROM xstatuses WHERE owner = '%s' AND number = '%s'" % (
            jabberID, number))
        ret = c.fetchone()
        if ret:
            (title) = ret[0]
    ...
    def setXstatusText(self, jabberID, number, title, desc):
        ...
        self.db_ping()
        c = self.db.cursor()
        c.execute("DELETE FROM xstatuses WHERE owner = '%s' AND number = '%s'" % (
            jabberID, number))
        c.execute("INSERT INTO xstatuses(owner,number,title,value) VALUES('%s','%s','%s','%s')" % (
            jabberID, number, title, desc))
    ...
    def getCSettingList(self, jabberID):
        ...
        c = self.db.cursor()
        c.execute(
            "SELECT variable,value FROM csettings WHERE owner = '%s'" % (jabberID))
        results = dict([])
        ret = c.fetchone()
        while ret:
            (variable) = ret[0]
    ...
    def getListEntry(self, type, jabberID, legacyID):
        ...
        attributes = {}
        c = self.db.cursor()
        c.execute("SELECT attribute,value FROM list_attributes WHERE owner = '%s' AND type = '%s' AND jid = '%s'" % (
            jabberID, type, legacyID))
        ret = c.fetchone()
        while ret:
    ...
    def getList(self, type, jabberID):
        ...
        c = self.db.cursor()
        c.execute(
            "SELECT jid FROM lists WHERE owner = '%s' AND type = '%s'" % (jabberID, type))
        ret = c.fetchone()
        while ret:
            (jid) = ret[0]
            entity = []
            ...
            self.db_ping()
            ic = self.db.cursor()
            ic.execute("SELECT attribute,value FROM list_attributes WHERE owner = '%s' AND type = '%s' AND jid = '%s'" % (
                jabberID, type, jid))
            iret = ic.fetchone()
            while iret:
    ...
    def setListEntry(self, type, jabberID, legacyID, payload={}):
        ...
        self.db_ping()
        c = self.db.cursor()
        c.execute("DELETE FROM lists WHERE owner = '%s' AND type = '%s' AND jid = '%s'" % (
            jabberID, type, legacyID))
        c.execute("DELETE FROM list_attributes WHERE owner = '%s' AND type = '%s' AND jid = '%s'" % (
            jabberID, type, legacyID))
        c.execute("INSERT INTO lists(owner,type,jid) VALUES('%s','%s','%s')" % (
            jabberID, type, legacyID))
        for p in payload.keys():
            c.execute("INSERT INTO list_attributes(owner,type,jid,attribute,value) VALUES('%s','%s','%s','%s','%s')" % (
                jabberID, type, legacyID, p, re.escape(payload[p].replace("'", "\\'"))))
