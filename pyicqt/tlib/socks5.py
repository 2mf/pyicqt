...
from pyicqt.tlib.sockserror import *
...
server.
    ...
    def setTcpNoDelay(self, enabled):
        # as possible implement it? SOCKS 5 protocol have no support for one
        pass
...
class ClientFactory (protocol.ClientFactory):
    ...
    def clientConnectionLost(self, connector, reason):
        ...
        try:
            if self.status != "established":
                ...
                #
                LogEvent(
                    ERROR, msg="Connection LOST before SOCKS established %s" % self)
    ...
    def clientConnectionFailed(self, connector, reason):
        ...
        try:
            if self.status != "established":
                LogEvent(
                    ERROR, msg="Connection FAILED before SOCKS established %s" % self)
