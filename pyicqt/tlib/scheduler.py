...
# it exited.
#
# I believe this will lead to two or three threads per online user after
# the startup period is over.
...
class Scheduler(object):
    ...
    def __init__(self, handler):
        ...
        self.threads = {
            'default': self.QueueThread('default', handler, self.freezer)}
    ...
    def enqueue(self, fam, sub, snac):
        ...
        if (not self.threads.has_key(classid) or not self.threads[classid].isAlive()):
            self.threads[classid] = self.QueueThread(
                classid, self.handler, self.freezer)
    ...
    def setStat(self, classid, window=-1, clear=-1, alert=-1, limit=-1, disconnect=-1, rate=-1, lasttime=-1, maxrate=-1):
        ...
        if (not self.threads.has_key(classid) or not self.threads[classid].isAlive()):
            self.threads[classid] = self.QueueThread(
                classid, self.handler, self.freezer)
        self.threads[classid].setStat(
            window=window, rate=rate, target=target, lasttime=lasttime, max=maxrate)
        self.bigLock.release()
    ...
    class QueueThread(threading.Thread):
        ...
        def setStat(self, window=-1, rate=-1, target=-1, lasttime=-1, max=-1):
            self.rm.setStat(
                window=window, rate=rate, target=target, lasttime=lasttime, max=max)
    ...
    class RateManager:
        ...
        def getDelay(self):
            ...
            """
            self.lock.acquire()
            nexttime = (self.window * self.target -
                        (self.window - 1) * self.rate) / 1000. + self.lasttime
            now = time.time()
            self.lock.release()
        ...
        def updateRate(self):
            ...
            if (self.window == -1):
                return
            now = time.time()
            newrate = (self.window - 1.) / self.window * self.rate + \
                1. / self.window * (now - self.lasttime) * 1000
            if (newrate > self.max):
                self.rate = self.max
            else:
                self.rate = newrate
