...
class JabberConnection:
    ...
    def __init__(self, pytrans, jabberID):
        ...
        self.last_el = dict()
        # Whether this user can accept typing notifications.
        self.typingUser = False
        # Whether this user can accept chat state notifications.
        self.chatStateUser = False
        # The ID of the last message the user sent to a particular contact.
        # Indexed by contact JID
    ...
    def checkFrom(self, el):
        ...
        return (froj.userhost() == self.jabberID)
    ...
    def sendMessage(self, to, fro, body=None, mtype=None, delay=None, xhtml=None, nickname=None, receipt=None, mID=None):
        ...
        if xhtml and not self.hasCapability(globals.XHTML):
            ...
            xhtml = None
        sendMessage(
            self.pytrans, to, fro, body, mtype, delay, xhtml, nickname, receipt, mID)
    ...
    def sendErrorMessage(self, to, fro, etype, explanation, condition=None, body=None):
        if self.last_el.has_key(to) and self.last_el[to].attributes.has_key("from"):
            ...
            sendErrorMessage(self.pytrans, to=to, fro=self.last_el[to].attributes[
                             "to"], etype=etype, condition=condition, explanation=explanation, body=body, el=self.last_el[to])
            del self.last_el[to]
        else:
            LogEvent(INFO, self.jabberID, "**NOT** Using pre-existing element")
            sendErrorMessage(self.pytrans, to=to, fro=fro, etype=etype,
                             condition=condition, explanation=explanation, body=body)
    ...
    def sendPresence(self, to, fro, show=None, status=None, priority=None, ptype=None, avatarHash=None, nickname=None, payload=[], url=None):
        ...
        LogEvent(INFO, self.jabberID)
        sendPresence(self.pytrans, to, fro, show, status,
                     priority, ptype, avatarHash, nickname, payload, url=url)
    ...
    def onMessage(self, el):
        ...
        if body and chatStates:
            ...
            self.chatStateReceived(
                toj.userhost(), toj.resource, chatStateEvent, withmessage=True)
            ...
            LogEvent(INFO, self.jabberID, "Chat state notification %r" %
                     chatStateEvent)
            self.chatStateReceived(
                toj.userhost(), toj.resource, chatStateEvent)
        elif not body and messageEvent:
            LogEvent(INFO, self.jabberID, "Typing notification %r" % composing)
            self.typingNotificationReceived(
                toj.userhost(), toj.resource, composing)
        ...
        if body:
            ...
            ), toj.resource, mtype, body, noerror, xhtml, autoResponse=autoResponse, jabber_mid=jabber_mid)
    ...
    def onPresence(self, el):
        ...
        if ptype and (ptype.startswith("subscribe") or ptype.startswith("unsubscribe")):
            LogEvent(
                INFO, self.jabberID, "Parsed subscription presence packet")
            ...
            if not ptype:
                ...
                if avatarHash and not config.disableAvatars:
                    self.avatarHashReceived(
                        froj.userhost(), toj.userhost(), avatarHash, avatarType)
                if nickname:
                    self.nicknameReceived(
                        froj.userhost(), toj.userhost(), nickname)
