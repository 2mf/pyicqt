...
def observer2(eventDict):
    ...
    if isinstance(edm, LogEvent):
        ...
        if not eventDict['isError'] and config.debugLevel < 3:
            return  # not error
        text = str(edm)
    else:
    ...
    timeStr = time.strftime(
        "[%Y-%m-%d %H:%M:%S]", time.localtime(eventDict['time']))
    text = text.replace("\n", "\n\t")
    global debugFile
    debugFile.write("%s %s\n" % (timeStr, text))
    debugFile.flush()
        ...
        if not self.skipargs:
            for key in self.args.keys():
                ...
                try:
                    ...
                    # If its not an object with length, assume that it can't be
                    # too big. Hope that's a good assumption.
                    pass
