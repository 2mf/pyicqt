...
from pyicqt import jabw
from pyicqt import contact
...
class Session(jabw.JabberConnection):
    ...
    def __init__(self, pytrans, jabberID, username, password, ulang, rosterID):
        ...
        # Whether session was closed actively (by us) or not
        self.active_close = None
        self.alive = True
        # Only ready when we're logged into the legacy service
        self.ready = False
        ...
        self.legacycon = legacy.LegacyConnection(
        ...
        if config.sessionGreeting:
            self.sendMessage(
                to=self.jabberID, fro=config.jid, body=config.sessionGreeting)
        ...
        if len(self.pytrans.sessions) + 1 > self.pytrans.serviceplugins['Statistics'].stats["MaxConcurrentSessions"]:
            self.pytrans.serviceplugins['Statistics'].stats[
                "MaxConcurrentSessions"] = len(self.pytrans.sessions) + 1
        self.pytrans.serviceplugins['Statistics'].sessionUpdate(
            self.jabberID, "Connections", 1)
    ...
    def removeMe(self):
        ...
        if self.pytrans:
            ...
            self.sendPresence(
                to=self.jabberID, fro=tmpjid, ptype="unavailable")
            self.pytrans.serviceplugins[
                'Statistics'].stats["OnlineSessions"] -= 1

        # Clean up stuff on the legacy service end (including sending offline
        # presences for all contacts)
        if self.legacycon:
    ...
    def doDiscoRequest(self):
        ...
        LogEvent(INFO, self.jabberID, "Fetching disco info from %r" %
                 self.jabberID)
        ...
        def vCardReceived(el):
            ...
            for e in vCard.elements():
                ...
                if e.name == "PHOTO" and not config.disableAvatars and not config.disableVCardAvatars and self.avatarPri < 2:
                    ...
                    if not imageData:
                        # Possibly it wasn't in a supported format?
                        errback(Exception("Invalid image data"))
    ...
    def doIQAvatarUpdate(self):
        if config.disableAvatars or config.disableIQAvatars:
            return
        ...
        def iqAvatarReceived(el):
            ...
            if qtype == "error":
                LogEvent(
                    INFO, self.jabberID, "That didn't work, let's try an IQ-storage-based avatar")
                d = self.sendStorageAvatarRequest(
                    to=self.jabberID, fro=config.jid)
        ...
        if not highestActive:
            return
        d = self.sendIQAvatarRequest(
            to=self.jabberID + "/" + highestActive, fro=config.jid)
    ...
    def sendNotReadyError(self, source, resource, dest, body):
        self.sendErrorMessage(self.pytrans, source + '/' + resource, dest,
                              "wait", "not-allowed", lang.get("waitforlogin", self.lang), body)

    def nicknameReceived(self, source, dest, nickname):
        if dest.find('@') > 0:
            return  # Ignore presence packets sent to users
    ...
    def messageReceived(self, source, resource, dest, destr, mtype, body, noerror, xhtml, autoResponse=0, jabber_mid=None):
        ...
        self.legacycon.sendMessage(
            dest, resource, body, noerror, xhtml, autoResponse=autoResponse, jabber_mid=jabber_mid)
    ...
    def chatStateReceived(self, dest, resource, state, withmessage=False):
        ...
        self.legacycon.chatStateNotification(
            dest, resource, state, withmessage)
    ...
    def presenceReceived(self, source, resource, to, tor, priority, ptype, show, status, url=None):
        ...
        # legacy services status. If there are no more resources then the
        # session is deleted
        if ptype == 'probe':
            ...
            self.handleResourcePresence(
                source, resource, to, tor, priority, ptype, show, status, url)

    def handleResourcePresence(self, source, resource, to, tor, priority, ptype, show, status, url):
        if ptype and ptype != "unavailable":
            return  # Ignore presence errors, probes, etc
        if to.find('@') > 0:
            return  # Ignore presence packets sent to users
        ...
        if ptype == "unavailable":
            if existing:
                ...
                # I don't know the resource, and they're leaving, so it's all
                # good
                return
        else:
            if not existing:
            ...
            self.resourceList[resource] = SessionResource(
                show, status, priority, url)

        highestActive = self.highestResource()

        if highestActive:
            # If we're the highest active resource, we should update the legacy
            # service
            ...
            LogEvent(
                INFO, self.jabberID, "Last resource died.  Calling removeMe in 0 seconds")
    ...
    def subscriptionReceived(self, to, subtype):
        ...
        if to.find('@') > 0:
            LogEvent(
                INFO, self.jabberID, "Passing subscription to legacy service")
            self.contactList.jabberSubscriptionReceived(to, subtype)
        else:
            if subtype == "subscribe":
                self.sendPresence(
                    to=self.jabberID, fro=config.jid, ptype="subscribed")
